...
const settings = definePluginSettings({
	receiveDirectMessageNotifications: {
		description:
			"Choose whether to receive notifications for received messages",
	...
	receiveFriendRequestNotifications: {
		description:
			"Choose whether to receive notifications for new friend requests",
		type: OptionType.BOOLEAN,
		default: false,
	},
	ignoreUsers: {
...
async function receiveMessage(message: Message, channelId: string) {
	...
	if (!settings.store.receiveNotificationsFromGroups && channel.isGroupDM())
		return;
	if (!settings.store.receiveDirectMessageNotifications && channel.isDM())
		return;

	const length = message.content.length ?? 0;
	let body: string = message.content;
	if (length > 138) {
		body = body.slice(0, 138) + "...";
	}
